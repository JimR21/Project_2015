...
static void processDefineSchema(DefineSchema *s){
  ...
  // add # of columns for every relation to schema
  schema.clear();
  schema.insert(schema.begin(),s->columnCounts,s->columnCounts+s->relationCount);

  for(i = 0; i < s->relationCount; i++)
    cout << " " << s->columnCounts[i] << " ";
  ...
  for(i = 0; i < s->relationCount; i++) {   // For every relation
      ...
      Journals.push_back(journal);          // Add Journal to Journals vector
  }
...
static void processTransaction(Transaction *t){
    ...
    for(i = 0; i < t->deleteCount; i++) {   // for all the delete operations
        ...
        for(int j=0;j<schema[o->relationId];j++)
            record->addValue(rec->getValue(j));  // add values to record from rec


        Journals[o->relationId]->insertJournalRecord(record);
  ...
  for(i=0; i < t->insertCount; i++) {
	...
	for(j=0;j< o->rowCount * schema[o->relationId] ;j++) {
        if(j % schema[o->relationId] == 0) {
            record = new JournalRecord(t->transactionId);
        }
		printf("%lu ", o->values[j]);
        record->addValue(o->values[j]);  // add values to record
