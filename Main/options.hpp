/*
            OPTIONS
1. Enable Transactions HashTable
2. Enable Validations HashTalbe
3. Enable Threads
4. Number of threads
5. Number of rounds till validations are flushed and printed
*/

#define TID_HASHTABLE 0     // PART 2: 1 enable, 0 disable Tid_HashTable
#define VAL_HASHTABLE 0     // PART 2: 1 enable, 0 disable Val_HashTable
#define VAL_THREADS 0 	    // PART 3: 1 enable, 0 disable threads
#define NUM_OF_THREADS 5    // Number of threads
#define ROUNDS 1		    // Default = 1. Number of rounds till validations are flushed and printed
